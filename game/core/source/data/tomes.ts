 ...
 *
 * 7 个 stat tomes（speed/attack_speed/life/consumable/attraction/shield/precision）输出 modifiers。
 * 5 个 contextual tomes（thorns/knockback/luck/xp_gain/curse）的效果在各自代码路径
 * 直接读 tome power（growth；旧数据 fallback 到 level），本表里 `modifiers: () => []`，仅作 metadata。
 *
 ...
 * - `maxHp`         → 玩家生命上限 (charCfg.hp + shopBonus 是 base)
 ...
 * - `consumableDropMult` → 消耗品掉落倍率 (base 1.0)
 *
...
export interface TomeDef {
   ...
   * 入参是 tome power（稀有度累计成长），不是单纯 level 次数。
   * Contextual tomes（thorns/knockback/luck/xp_gain/curse）返回空数组。
   */
  modifiers(level: number): readonly Modifier[];
  /**
   * 标记 contextual tome —— 文档 / debug 用。代码路径通过 player.tomes.find + getTomePower 读取强度，
...
// ─────────────────────────────────────────────────────────────────────────
// 7 个 stat tomes —— 走 stat pipeline
// ─────────────────────────────────────────────────────────────────────────
...
const lifeTome: TomeDef = {
  type: 'life_tome',
  maxLevel: 8,
  modifiers: (lv) => [{ kind: 'added', stat: 'maxHp', value: lv * 15 }],
  description: '生命上限 +15/级',
  tags: ['defense'],
};

const consumableTome: TomeDef = {
  type: 'consumable_tome',
  maxLevel: 8,
  modifiers: (lv) => [{ kind: 'increased', stat: 'consumableDropMult', value: lv * 0.05 }],
  description: '消耗品掉落率 +5%/级（乘算）',
  tags: ['utility'],
};
...
export const TOMES: Record<TomeType, TomeDef> = {
  ...
  life_tome: lifeTome,
  consumable_tome: consumableTome,
...
export const TOME_MAX_LEVELS_FROM_DATA: Record<TomeType, number> = {
  ...
  life_tome: TOMES.life_tome.maxLevel,
  consumable_tome: TOMES.consumable_tome.maxLevel,
